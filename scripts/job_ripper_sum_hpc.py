...
def main(argv):
    parser = argparse.ArgumentParser(description="RIPPER summary job (HEROS-compatible outputs)")

    parser.add_argument("--o", dest="outputPath", type=str, required=True,
                        help="Path to RIPPER_<analysis> output folder (contains dataset subfolders)")
    ...
    # 1) Ensure evaluation_summary.csv exists per cv folder
    ...
    # 3) Dataset-level seed summaries
    for entry in os.listdir(outputPath):
        data_level_path = outputPath / entry
        if not data_level_path.is_dir():
            continue
    ...
    if make_plots:
        for entry in os.listdir(outputPath):
            ...
            if "test_accuracy" in df_all.columns:
                ...
                plt.title("Testing Accuracy (All Runs)")
